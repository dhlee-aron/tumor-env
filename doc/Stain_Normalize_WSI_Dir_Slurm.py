...
if __name__ =='__main__':
    ...
    if stain_mode == 'R':
        ...
        print("Stain mode: Reinhard")
    elif stain_mode == 'M':
        n = stainNorm_Macenko.Normalizer()
        print("Stain mode: Macenko")
    elif stain_mode == 'V':
        n = stainNorm_Vahadane.Normalizer()
        print("Stain mode: Vahadane")
    else:
        n = stainNorm_Vahadane.Normalizer()
        print("Default stain mode: Vahadane")
    n.fit(ref_image)

    wsi_names = [f for f in os.listdir(wsi_ori_dir)]

    wsi_names = wsi_names[range_start:range_end] #135 295
    print(range_start)
